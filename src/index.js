...
app.post('/api/render', async (req, res) => {
  try {
    const { json, platform, image_urls } = req.body;

    if (!json || !platform) {
      return res.status(400).json({ error: 'json과 platform은 필수' });
    }

    console.log(`[RENDER] 시작: ${json.hero?.product_title || 'unknown'} (${platform}), 이미지: ${(image_urls||[]).length}장`);
    const start = Date.now();

    const result = await renderSlides(json, platform, image_urls || []);
